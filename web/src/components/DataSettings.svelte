...
<div class="panel"><div class="panel-heading"><div><p class="workspace-kicker">{zh ? '掌控你的资料' : 'OWN YOUR DATA'}</p><h2>{zh ? '数据管理' : 'Your data'}</h2></div></div>
  <div class="data-row"><div><h3>{zh ? '导出数据' : 'Export data'}</h3><p>{zh ? '下载包含档案、目标、回顾、成果、记忆、待办和对话的 JSON 文件。' : 'Download your profile, goals, check-ins, deliverables, memories, tasks, and conversation as JSON.'}</p></div><button class="ws-secondary" onclick={download}>{zh ? '导出数据' : 'Export data'}</button></div>
  ...
  {#if importPreview}<div class="confirm-box"><p>{zh ? `将导入 ${importPreview.memories.length} 条记忆、${importPreview.tasks.length} 条待办和 ${importPreview.deliverables.length} 份成果，并替换当前数据。` : `Import ${importPreview.memories.length} memories, ${importPreview.tasks.length} tasks, and ${importPreview.deliverables.length} deliverables, replacing current data.`}</p><button class="ws-primary" onclick={replace}>{zh ? '确认替换' : 'Confirm replace'}</button><button class="ws-secondary" onclick={() => importPreview = null}>{zh ? '取消' : 'Cancel'}</button></div>{/if}
