<script lang="ts">
  ...
  let tab: 'home' | 'goals' | 'chat' | 'profile' | 'memories' | 'tasks' | 'deliverables' | 'data' = 'home';
  let starterGoalId: string | undefined;
  let error = '';
  let apiKey = '';
  ...
  function startGoal(goalId: string) { starterGoalId = goalId; tab = 'chat'; }
  const tabs = [
    ...
    { id: 'deliverables', en: 'Deliverables', zh: '成果', icon: '▤' },
...
{:else}
  <main class="workspace" aria-label={zh ? '个人工作台' : 'Personal workspace'}>
    <aside class="workspace-sidebar"><div class="sidebar-heading"><span class="sidebar-agent-icon">✳</span><div><strong>{zh ? '我的 Agent' : 'My Agent'}</strong><small>{zh ? '个人工作台' : 'PERSONAL WORKSPACE'}</small></div></div>
      <nav aria-label={zh ? '工作台导航' : 'Workspace navigation'}>{#each tabs as item}<button class:active={tab === item.id} onclick={() => { starterGoalId = undefined; tab = item.id; }}><span class="tab-icon">{item.icon}</span>{zh ? item.zh : item.en}</button>{/each}</nav>
    ...
    <div class="workspace-main"><div class="workspace-topbar"><span>{zh ? '你的个人空间' : 'YOUR PERSONAL SPACE'}</span><span>{data.profile.name}</span></div>
      ...
      {#if tab === 'home'}
        ...
        <AgendaPanel {locale} {data} onStart={startGoal} />
      ...
      {:else if tab === 'chat'}
        <Conversation {locale} {data} {apiKey} {providerConfig} {starterGoalId} onKeyChange={setApiKey} onProviderChange={setProvider} onUrlChange={setApiUrl} onModelChange={setModel} onSave={persist} />
      {:else if tab === 'profile'}
      ...
      {:else if tab === 'deliverables'}
        <Deliverables {locale} {data} onSave={persist} onGoal={() => tab = 'goals'} />
      {:else}
